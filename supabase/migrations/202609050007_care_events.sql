-- Apply before the new client. Existing clients must reload to read these event types.
...
alter table public.tracking_events add constraint tracking_events_type_check
  check (type in ('breast', 'bottle', 'sleep', 'diaper', 'vaccination', 'medication', 'meal', 'growth', 'activity'));
...
begin
  ...
  if jsonb_typeof(p_event) is distinct from 'object'
    ...
    or coalesce(p_event->>'type', '') not in ('breast', 'bottle', 'sleep', 'diaper', 'vaccination', 'medication', 'meal', 'growth', 'activity')
