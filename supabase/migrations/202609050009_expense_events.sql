-- Apply before the new client. Existing clients must reload to read expense events.
-- Keep the scoped outbox/revision protocol; no new tables or direct-write permissions.
begin;
...
alter table public.tracking_events add constraint tracking_events_type_check
  check (type in ('breast', 'bottle', 'sleep', 'diaper', 'vaccination', 'medication', 'meal', 'growth', 'activity', 'expense'));
...
begin
  ...
  if p_type = 'expense' then
    return p_end is null
      and p_payload ?& array['amount', 'category', 'custom_category', 'title', 'payer', 'method']
      and p_payload - array['amount', 'category', 'custom_category', 'title', 'payer', 'method'] = '{}'::jsonb
      and jsonb_typeof(p_payload->'amount') = 'number'
      and (p_payload->>'amount')::numeric = trunc((p_payload->>'amount')::numeric)
      and (p_payload->>'amount')::numeric between 1 and 1000000000
      and (p_payload->>'category') in ('milk', 'diaper', 'food', 'health', 'clothes', 'education', 'toys', 'other', 'custom')
      and jsonb_typeof(p_payload->'custom_category') = 'string' and length(p_payload->>'custom_category') <= 40
      and case when p_payload->>'category' = 'custom'
        then length(btrim(p_payload->>'custom_category', E' \t\n\r')) > 0
          and p_payload->>'custom_category' = btrim(p_payload->>'custom_category', E' \t\n\r')
        else p_payload->>'custom_category' = '' end
      and jsonb_typeof(p_payload->'title') = 'string' and length(p_payload->>'title') <= 120
      and (p_payload->>'payer') in ('father', 'mother', 'other')
      and (p_payload->>'method') in ('cash', 'transfer', 'card', 'ewallet');
  end if;
  if p_type = 'medication' then
    return p_end is null
      and p_payload ?& array['name', 'dose', 'status']
      and p_payload - array['name', 'dose', 'status'] = '{}'::jsonb
      and jsonb_typeof(p_payload->'name') = 'string'
      and length(btrim(p_payload->>'name', E' \t\n\r')) > 0 and length(p_payload->>'name') <= 120
      and jsonb_typeof(p_payload->'dose') = 'string' and length(p_payload->>'dose') <= 80
      and (p_payload->>'status') in ('planned', 'completed');
  end if;
  if p_type = 'meal' then
    return p_end is null and p_payload ?& array['food', 'amount']
      and p_payload - array['food', 'amount'] = '{}'::jsonb
      and jsonb_typeof(p_payload->'food') = 'string'
      and length(btrim(p_payload->>'food', E' \t\n\r')) > 0 and length(p_payload->>'food') <= 160
      and jsonb_typeof(p_payload->'amount') = 'string' and length(p_payload->>'amount') <= 80;
  end if;
  if p_type = 'growth' then
    return p_end is null and p_payload ?& array['height_cm', 'weight_kg']
      and p_payload - array['height_cm', 'weight_kg'] = '{}'::jsonb
      and (p_payload->'height_cm' <> 'null'::jsonb or p_payload->'weight_kg' <> 'null'::jsonb)
      and (p_payload->'height_cm' = 'null'::jsonb or (jsonb_typeof(p_payload->'height_cm') = 'number'
        and (p_payload->>'height_cm')::numeric > 0 and (p_payload->>'height_cm')::numeric <= 250))
      and (p_payload->'weight_kg' = 'null'::jsonb or (jsonb_typeof(p_payload->'weight_kg') = 'number'
        and (p_payload->>'weight_kg')::numeric > 0 and (p_payload->>'weight_kg')::numeric <= 300));
  end if;
  if p_type = 'activity' then
    return p_end is null and p_payload ?& array['kind', 'duration_minutes']
      and p_payload - array['kind', 'duration_minutes'] = '{}'::jsonb
      and (p_payload->>'kind') in ('bath', 'tummy_time', 'outdoor', 'indoor', 'brushing_teeth')
      and (p_payload->'duration_minutes' = 'null'::jsonb or (jsonb_typeof(p_payload->'duration_minutes') = 'number'
        and (p_payload->>'duration_minutes')::numeric > 0 and (p_payload->>'duration_minutes')::numeric <= 1440));
  end if;
...
-- Only planned vaccinations/medication can be future appointments.
-- Authentication, locks, immutable types, conflicts, idempotency and cursors are unchanged.
...
begin
  ...
  if jsonb_typeof(p_event) is distinct from 'object'
    ...
    or coalesce(p_event->>'type', '') not in ('breast', 'bottle', 'sleep', 'diaper', 'vaccination', 'medication', 'meal', 'growth', 'activity', 'expense')
  ...
  if private.valid_payload(p_event->>'type', v_start, v_end, p_event->'payload') is not true
    or (v_start > now() + interval '5 minutes'
      and not (p_event->>'type' in ('vaccination', 'medication') and p_event->'payload'->>'status' = 'planned'))
