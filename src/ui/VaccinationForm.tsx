...
export function VaccinationForm({ body, initialStatus, timezone, saving, onSave, onDelete }: {
  ...
  return <form className="stack" onSubmit={event => {
    ...
    <div className="row"><label>{draft.status === 'planned' ? 'Ngày dự kiến' : 'Ngày đã tiêm'}<input name="date" type="date" required value={draft.date}
      max={draft.status === 'completed' ? dayKey(Date.now(), timezone) : undefined} disabled={saving} onChange={event => setDraft({ ...draft, date: event.target.value })} /></label>
